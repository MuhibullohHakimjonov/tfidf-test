...
async function fetchDocuments(page = 1) {
  ...
  try {
    ...
    const response = await axios.get(`documents/?page=${page}`, {
...
async function fetchCollections() {
  try {
    const response = await axios.get(`collections/?page=1`, {
      withCredentials: true
    });
...
async function deleteDocument(docId) {
  try {
    await axios.delete(`documents/${docId}/delete/`, { withCredentials: true });
...
async function addToCollection() {
  ...
  try {
    await axios.post(
      `collection/${selectedCollectionId.value}/${selectedDocumentId.value}/`,
      {}, { withCredentials: true }
    );
    closeModal();
  } catch (error) {
