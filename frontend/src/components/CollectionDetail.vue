...
const fetchCollection = async () => {
  try {
    const response = await axios.get(`http://37.9.53.228/api/collections/${route.params.id}/`);
...
const fetchCollectionStatistics = async () => {
  try {
    const response = await axios.get(`http://37.9.53.228/api/collections/${route.params.id}/statistics/`);
    ...
    if (!collection.value.documents_id.name) {
      const collResponse = await axios.get(`http://37.9.53.228/api/collections/${route.params.id}/`);
...
const removeDocument = async (documentId) => {
  ...
  try {
    await axios.delete(`http://37.9.53.228/api/collection/${route.params.id}/${documentId}/delete/`);
